...
## 🤖 Telegram Bot 中文自然語言指令說明

在 Telegram 聊天視窗中可直接輸入以下**中文指令**（無需打斜線 `/`）：

| 中文指令 | 說明 | 範例 |
| :--- | :--- | :--- |
| **`指令`** | 顯示所有指令說明選單 | `指令` |
| **`新增關鍵字 <看板> <關鍵字>`** | 新增指定看板的關鍵字（支援逗號多組） | `新增關鍵字 Stock 台積電,聯發科` |
| **`刪除關鍵字 <看板> <關鍵字>`** | 刪除指定看板的關鍵字 | `刪除關鍵字 Stock 台積電` |
| **`新增推文數 <看板> <推文數>`** | 設定指定看板的推文達標推播門檻 | `新增推文數 Stock 50`<br>`新增推文數 Gossiping 80` |
| **`刪除推文數 <看板>`** | 移除指定看板的推文門檻監控 | `刪除推文數 Stock` |
| **`清單`** | 顯示目前各看板監控中的關鍵字與推文數 | `清單` |
| **`狀態`** | 查詢系統運作狀態、監控看板數與推播總數 | `狀態` |
| **`停止監控`** | 暫停爬蟲與推播（開會/睡覺時） | `停止監控` |
| **`開始監控`** | 開始/恢復即時監控與推播 | `開始監控` |
